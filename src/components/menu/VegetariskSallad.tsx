...
const VegetariskSallad = () => {
  ...
  return (
    <section
      ...
      className="max-w-7xl mx-auto px-4 sm:px-6 py-24 flex flex-col-reverse lg:flex-row items-center lg:items-start gap-12 text-center lg:text-right"
    >
      {/* Left: Menu Items + Filler images */}
      <div className="flex flex-col items-center lg:items-start w-full lg:w-2/3">
        ...
        <motion.div
          ...
          className="relative w-full z-10 -mt-6 sm:-mt-10 lg:-mt-20"
        >
          {/* Scrollable wrapper (scroll solo su mobile) */}
          <div className="flex sm:grid sm:grid-cols-2 gap-6 bg-burger-bun p-4 sm:p-6 rounded-2xl shadow-inner overflow-x-auto sm:overflow-x-hidden scrollbar-hide">
            {vegetariskSalladData.map((item: VegetariskSalladMenu) => (
              <div
                key={item.id}
                className="flex sm:flex-row flex-col items-start gap-4 bg-white rounded-2xl shadow-md p-4 min-w-[260px] sm:min-w-0 flex-shrink-0"
              >
                <img
                  src={item.img}
                  alt={item.name}
        ...
        {/* All filler images below menu */}
        <div className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
          {vegetariskSalladData.map((item, index) => (
            <img
              key={index}
              src={item.img}
              alt={`Extra veg ${item.name}`}
